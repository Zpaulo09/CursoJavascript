...
function validarCadastro() {
    ...
    const senhaConfirmacao = mensagemErroConfirmacao.value
    ...
    if(senha.length <= 6){
        issenhaValid = false
        menssagemErro.textContent = "A senha deve ter no mínimo 6 caracteres."
    }

    if(!issenhaValid && senha == senhaConfirmacao){
        issenhaValid = false
        mensagemErro.textContent = "As senhas devem coincidir"
    }
    else if (senhaRegex.test(senha)){
        mensagemErro.textContent = "Senha Válida"
        issenhaValid = true
    }
    else {
